...
public abstract class Living 
{
	...
	protected void census(int population[ ]){
		...
		int minimum_row = row -1;
		int maximum_row = row +1;
		int minimum_column = column - 1;
		int maximum_column = column + 1;
		if (minimum_column < 0){
			minimum_column = 0;
		}
		if (minimum_row < 0){
		...
		if(maximum_row > plain.getWidth() - 1){
			maximum_row -= 1;
		}
		if(maximum_column > plain.getWidth() - 1){
			maximum_column -= 1;
		}
		for(int r = minimum_row; r <= maximum_row; r++){
			for(int c = minimum_column; c <= maximum_column; c++){
				State inst = plain.grid[r][c].who(); //gets the instance on what state of the animal
				if(inst == State.BADGER){
					population[BADGER] += 1;
				}
				else if(inst == State.EMPTY){
					population[EMPTY] += 1;
				}
				else if (inst == State.FOX){
					population[FOX] += 1;
				}
				else if (inst == State.GRASS){
					population[GRASS] += 1;
				}
				else if(inst == State.RABBIT){
					population[RABBIT] += 1;
				}
			}
