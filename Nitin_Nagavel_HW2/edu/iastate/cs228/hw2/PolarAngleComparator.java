...
public class PolarAngleComparator implements Comparator<Point>
{
	...
	public int compare(Point p1, Point p2)
	{
		if(p1.equals(p2))
			...
			return -1;
		else if (compareDistance(p1, p2) == -1 && comparePolarAngle(p1, p2) == 0)
			return -1;
		else
			return 1;
	}
    ...
    {
    	if((p1.equals(p2)) ||
				(!(p1.equals(referencePoint)) &&
						!(p2.equals(referencePoint)) && (dotProduct(p1, p2) >= 0) && (crossProduct(p1,p2) == 0))){
    		return 0;
		}
    	else if(p1.equals(referencePoint)){
    		return -1;
    ...
    {
    	int p1_1 = dotProduct(p1, p1);
    	int p2_1 = dotProduct(p2, p2);
    	int distance = p1_1 - p2_1; //if p1_1 is has a shorter distance than p2_1 it returns a negative
    	if (distance == 0){
    		return 0;
		}
    	else if(distance < 0){
    ...
    {
		Point first = new Point(p1.getX() - referencePoint.getX(), p1.getY() - referencePoint.getY()); //Distance subtraction between the reference and the point
		Point second = new Point(p2.getX() - referencePoint.getX(), p2.getY() - referencePoint.getY()); //Distance subtraction between the
		int crossprd =  (first.getX() * second.getY()) - (second.getX() * first.getY());
		return crossprd;
    ...
    {
    	//Physics math gang!
		Point first = new Point(p1.getX() - referencePoint.getX(), p1.getY() - referencePoint.getY()); //basically subtracts the distance between the first point and the reference
		Point second = new Point(p2.getX() - referencePoint.getX(), p2.getY() - referencePoint.getY());//subtracts the distance between the second point and the reference
		int dotprd = (first.getX() * second.getX() + first.getY() * second.getY());
		return dotprd;
	}
}
