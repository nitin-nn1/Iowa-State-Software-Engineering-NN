...
public abstract class AbstractSorter
{
     ...
     * 
     * If order == 2, the method cannot be called when referencePoint == null.  Call setReferencePoint()
	...
	public void setComparator(int order) throws IllegalArgumentException, IllegalStateException
	{
		...
		if (order == 0){
			...
			pointComparator = new Comparator<Point>() {
				@Override
				public int compare(Point p11, Point p22) {
					return p11.compareTo(p22);
				}
			};
		}
		else if(order == 1){
		...
		else if (order == 2){ //If the Method of SetReferencePoint is already called, and the reference point is called.
